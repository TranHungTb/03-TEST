...
void Connect_Server(String ip, String port);
...
//  Gsm_MakeCall(myphone);                          // Test cuoc goi 
  ...
  Connect_Server("138.197.46.187","1346");
...
void loop() {
  ...
  delay(1000);                                                                    // tre 1s 
  Index_Lamp_On = ab.indexOf("GMT");              // Tim vi tri cua chuoi "LAMP_ON" trong bo dem nhan RxBuff
...
void Connect_Server(String ip, String port)
{
  ...
  serialEvent();
    delay(1000);                                                                    // tre 1s 
  Index_Lamp_On = ab.indexOf("1");              // Tim vi tri cua chuoi "LAMP_ON" trong bo dem nhan RxBuff
  ...
  mySerial.println("AT+CIPMUX?");     //Kiểm tra xem Sim800A đang ở chế độ Single hay Multi Connection.
                                // X = 0   // None Transparent  

                                  //X = 1   // Transparent
  delay(2000); 
  updateSerial();
  mySerial.println("AT+CIPMode?");
  delay(2000);
  updateSerial();
  ...
  serialEvent();
    delay(1000);                                                                    // tre 1s 
  Index_Lamp_On = ab.indexOf("server");              // Tim vi tri cua chuoi "LAMP_ON" trong bo dem nhan RxBuff
 
  if(Index_Lamp_On >= 0)                                                   // Neu tim thay "LAMP_ON" trong RxBuff
  {
    Index_Lamp_On = -1;                                                     // 
    Serial.println("connect ok:" + ab);
    ab = "";                                                                   // Xoa bo dem
    digitalWrite(RELAY, HIGH);                                             // Dong Relay de bat den // Bat bong den
  }
  else
